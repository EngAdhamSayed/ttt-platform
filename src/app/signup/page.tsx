...
import { Loader2, Mail, Lock, User, Eye, EyeOff, Sparkles, KeyRound } from "lucide-react";
...
export default function SignupPage() {
  ...
  // 1️⃣ إنشاء الحساب (إرسال الـ OTP)
  const handleSignup = async (e: React.FormEvent) => {
    ...
    if (error) {
      ...
      // لو الـ Confirm email مقفول يدخل فوراً، لو شغال يتنقل لخطوة الـ OTP
      if (data.session) {
        router.push("/");
        router.refresh();
      } else {
  ...
  // 2️⃣ التحقق من كود الـ OTP المكون من 6 أرقام
    ...
    <div className="h-screen w-screen overflow-hidden bg-[#faf8f5] text-slate-900 flex flex-col justify-between items-center p-4 dir-rtl font-sans select-none">
      ...
      <div className="bg-white p-6 rounded-3xl border border-orange-100 shadow-sm w-full max-w-sm space-y-4 text-right my-auto">
        <div className="flex flex-col items-center justify-center space-y-1.5 text-center">
          ...
          <h1 className="text-xl font-black text-slate-900 tracking-wide">إنشاء حساب جديد</h1>
          <p className="text-[10px] font-bold text-orange-600">انضم إلى مجتمع TTT Platform</p>
        </div>
        ...
        {step === "form" ? (
          ...
          <form onSubmit={handleVerifyOtp} className="space-y-3">
            <p className="text-xs text-center text-slate-500">
              أدخل رمز التحقق (OTP) المكون من 6 أرقام المرسل إلى: <br />
            ...
            <div className="relative">
              <input
                ...
                className="w-full bg-slate-50 border border-slate-200 rounded-2xl p-3 text-sm text-center tracking-widest font-bold focus:outline-none focus:border-orange-500 focus:bg-white transition"
              />
            ...
            <button
              ...
              className="w-full text-xs text-slate-400 hover:underline text-center block pt-1"
            >
